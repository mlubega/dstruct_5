...
public class TestHash {
	...
	//	System.out.println(allEntries.size());
		...
		for (int i=0; i< (allEntries.size()/2); i++) {
			//System.out.println("BEFORE CALLING INSERT " +allEntries.get(i));
	...
	private static void testTimes(PrintStream out){
    	...
    	for (int i=numItems; i<allEntries.size(); i++) {
			...
			if (table.delete(allEntries.get(i)) == null) {
				System.out.println("Entry: " + allEntries.get(i).toString());
			}
		}
    	end = System.currentTimeMillis();
